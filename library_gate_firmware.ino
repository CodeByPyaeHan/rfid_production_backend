...
#define LED_GREEN    26
#define LED_RED      27
#define LED_YELLOW   14   // NFC1 (start) mode indicator
...
const int MAX_NFC2_READS = 2; // တစ်ကြိမ် max 2 ကတ်
...
// RF switch settle (crosstalk လျှော့ရန်)
const unsigned long RF_SETTLE_MS = 5;
// stayQuiet ပြီးနောက် နောက်ကတ် inventory အတွက်
const unsigned long QUIET_SETTLE_MS = 15;

// Both readers now block a scanned card for 1 minute on BOTH readers
const unsigned long BLOCK_DURATION_MS = 60000;

// ==========================================
// [ Reader Mode State Machine ]
// ==========================================
// MODE_NFC2 = default/idle state -> only NFC2 is powered (multi-read gate)
// MODE_NFC1 = "start" state       -> only NFC1 is powered (login scan)
enum ReaderMode { MODE_NFC2, MODE_NFC1 };
ReaderMode currentMode = MODE_NFC2;
...
struct BlockedTag {
  uint8_t uid[8];
  unsigned long expireTimeNFC1;
  unsigned long expireTimeNFC2;
};
#define MAX_BLOCKED_TAGS 40
BlockedTag blockedTags[MAX_BLOCKED_TAGS];

// ==========================================
// [ Feedback System ] — မူရင်းအတိုင်း (buzzer မပြောင်း)
// ==========================================
...
void startFeedback(FeedbackType type, int beeps) {
  ...
  // NOTE: LED_YELLOW is reserved for reader-mode indication and is
...
// ==========================================
// [ RF Control — one active reader at a time ]
// ==========================================
void activateReader(PN5180ISO15693& active, PN5180ISO15693& other) {
  other.setRF_off();
  delay(RF_SETTLE_MS);
  active.setupRF();
  delay(RF_SETTLE_MS);
...
// Switch to NFC1 ("start" command from backend) — used for login scan
void switchToNFC1() {
  activateReader(nfc1, nfc2);
  currentMode = MODE_NFC1;
  digitalWrite(LED_YELLOW, HIGH);
  // reset per-mode loop bookkeeping so we don't act on stale poll timers
  lastNFCPoll = 0;
  Serial.println(">>> [MODE] Switched to NFC1 (start) — NFC2 OFF, NFC1 ON");
}

// Switch to NFC2 (default / "close" command from backend) — gate multi-read
void switchToNFC2() {
  activateReader(nfc2, nfc1);
  currentMode = MODE_NFC2;
  digitalWrite(LED_YELLOW, LOW);
  lastNFCPoll = 0;
  Serial.println(">>> [MODE] Switched to NFC2 (close/default) — NFC1 OFF, NFC2 ON");
}
...
bool sameUID(uint8_t *a, uint8_t *b) {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // ---- Reader mode control from backend ----
  if (strcmp(action, "start") == 0) {
    Serial.println(">>> [MQTT] 'start' command received from backend.");
    if (currentMode != MODE_NFC1) {
      switchToNFC1();
    }
    return;
  }
  ...
  if (strcmp(action, "close") == 0) {
    Serial.println(">>> [MQTT] 'close' command received from backend.");
    if (currentMode != MODE_NFC2) {
      switchToNFC2();
    }
    return;
  }

  // ---- Feedback (buzzer / LED) from backend ----
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n>>> ESP32 Library Gate System Starting (Mode-Switched NFC1/NFC2)...");
  ...
  // Default state on boot: NFC2 active (gate), NFC1 off, waiting for "start"
  switchToNFC2();
...
void loop() {
  ...
  if (currentMillis - lastPN5180Maintenance >= PN5180_MAINTENANCE_INTERVAL) {
    ...
    Serial.println(">>> [MAINT] PN5180 reset");
    nfc1.reset();
    nfc2.reset();
    bothRFOff();
    // Re-apply whichever mode we were in before the maintenance reset
    if (currentMode == MODE_NFC1) switchToNFC1();
    else switchToNFC2();
  }
  ...
  // ========================================
  // MODE_NFC1 — "start" state: login scan only
  // ========================================
  if (currentMode == MODE_NFC1) {
    if (nfc1.getInventory(uid) == ISO15693_EC_OK) {
      if (!isTagBlocked(uid, 1)) {
        ...
        if (atIndex == -1 || rawData.length() == 0) {
          ...
          // Block this card on BOTH readers for 1 minute
          blockTag(uid, BLOCK_DURATION_MS, BLOCK_DURATION_MS);
  ...
  // ========================================
  // MODE_NFC2 — default/"close" state: multi-read gate
  // ========================================
  int nfc2LoopGuard = 0;
  int validCardsFoundInNfc2 = 0;
  bool nfc2HadActivity = false;

  // ဒီ cycle ထဲမှာ ဖတ်ပြီးသား UID များ (တူတာ ပြန်မဖတ်ရန်)
  uint8_t seenUid[MAX_NFC2_READS][8];
  int seenCount = 0;

  while (nfc2LoopGuard < MAX_NFC2_ITER && validCardsFoundInNfc2 < MAX_NFC2_READS) {
    nfc2LoopGuard++;
    esp_task_wdt_reset();
    mqttClient.loop();
    handleFeedback(); // buzzer timing မပျက်အောင်

    // If the backend sent "start" mid-loop, bail out of NFC2 scanning immediately
    if (currentMode != MODE_NFC2) {
      break;
    }

    if (nfc2.getInventory(uid) != ISO15693_EC_OK) {
      break; // ကတ်မရှိတော့ / inventory မရ
    }

    // ဒီ cycle မှာ ပြီးသား UID ဖြစ်ရင် quiet ထပ်လုပ်ပြီး ရှေ့ဆက်
    bool alreadySeen = false;
    for (int s = 0; s < seenCount; s++) {
      if (sameUID(uid, seenUid[s])) {
        alreadySeen = true;
        break;
      }
    }
    if (alreadySeen || isTagBlocked(uid, 2)) {
      nfc2.stayQuiet(uid);
      delay(QUIET_SETTLE_MS);
      continue;
    }
    ...
    if (atIndex == -1 || rawData.length() == 0) {
      ...
      startFeedback(FB_FAIL, 3); // invalid = local fail beep (မူရင်းအတိုင်း)
      blockTag(uid, 5000, 5000);
      nfc2.stayQuiet(uid);
      delay(QUIET_SETTLE_MS);
      nfc2HadActivity = true;
    } else {
      String validData = rawData.substring(atIndex);
      Serial.print(">>> [NFC2] Valid Card Data: ");
      Serial.println(validData);
      Serial.print(">>> [NFC2] SUCCESS! Card ");
      Serial.print(validCardsFoundInNfc2 + 1);
      ...
      // seen list ထည့်
      if (seenCount < MAX_NFC2_READS) {
        memcpy(seenUid[seenCount], uid, 8);
        seenCount++;
      }

      nfc2.stayQuiet(uid);
      delay(QUIET_SETTLE_MS); // နောက်ကတ် inventory အတွက် quiet settle
      validCardsFoundInNfc2++;
      nfc2HadActivity = true;
    }
  }

  if (validCardsFoundInNfc2 > 0) {
    Serial.print(">>> [NFC2] Multi-read done. Valid cards this cycle: ");
    Serial.println(validCardsFoundInNfc2);
  }

  // cooldown ကို multi-read အပြီးမှ တစ်ခါတည်း (loop ထဲမထည့် — 2 ကတ်ဖတ်နိုင်အောင်)
  if (nfc2HadActivity) {
