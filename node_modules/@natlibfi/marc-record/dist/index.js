...
export class MarcRecord {
  ...
  constructor(record, validationOptions = {}) {
    ...
    if (record) {
      ...
      recordClone.fields.filter(({ subfields }) => subfields).forEach((field) => {
  ...
  insertField(field, index) {
    ...
    return this;
    function format(field2) {
      const cloned = clone(field2);
    ...
    function convertFromArray(args) {
      if (field.length === 2) {
        const [tag2, value] = args;
        return { tag: tag2, value };
      }
      const [tag, ind1, ind2] = args;
      const subfields = parseSubfields(args.slice(3));
      return { tag, ind1, ind2, subfields };
      function parseSubfields(args2, subfields2 = []) {
        const [code, value] = args2;
        if (code) {
          return parseSubfields(args2.slice(2), subfields2.concat({ code, value }));
        }
        return subfields2;
      }
    }
  ...
  toObject() {
    return Object.entries(clone(this)).filter(([k]) => k.startsWith("_") === false).reduce((acc, [k, v]) => ({ ...acc, [k]: v }), {});
  }
  static fromString(str, validationOptions) {
    const record = new MarcRecord(void 0, validationOptions);
    str.split("\n").map((ln) => ({
      tag: ln.substr(0, 3),
      ind1: ln.substr(4, 1),
      ind2: ln.substr(5, 1),
    ...
    return record;
    function parseSubfields(str2) {
      return str2.substr(1).split("\u2021").map((data) => {
